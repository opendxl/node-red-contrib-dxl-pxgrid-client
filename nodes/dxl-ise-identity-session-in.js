/**
 * @module DxlIseIdentitySessionIn
 * @description Implementation of the `dxl-ise-identity-session in` node
 * @private
 */
...
var ISE_EVENT_IDENTITY_SESSION_TOPIC = util.ISE_IDENTITY_EVENT_PREFIX +
  'session'

module.exports = function (RED) {
  /**
   * @classdesc Node which registers to receive "session" events from Cisco
   ...
   * @constructor
   */
  function IseIdentitySessionInNode (nodeConfig) {
    ...
    if (this._client) {
      ...
      this._client.addEventCallback(ISE_EVENT_IDENTITY_SESSION_TOPIC, callback)
      this.on('close', function (done) {
        node._client.removeEventCallback(ISE_EVENT_IDENTITY_SESSION_TOPIC,
          callback)
